/*
  PKE Meter with Teensy 4.0 and LIS3MDL Magnetometer
  Original code for MetroMini 5V adapted for Teensy 4.0
  Pin Definitions:
  - SERVO_PIN: Pin 11
  - SOUND_POT_PIN: Pin A1
  - VOLUME_POT_PIN: Pin A2
  - LIS3MDL_I2C: SDA (Pin 18), SCL (Pin 19)
...
// Uncomment for debug mode and PWM Servo usage
//#define DEBUG_MODE
#define USE_PWM_SERVO

#ifdef DEBUG_MODE
#define DEBUG_PRINTLN(x, d) { Serial.println(x); delay(d); }
#else
...
Adafruit_MPR121 touchSensor = Adafruit_MPR121(); // (i2c)
DFRobotDFPlayerMini myDFPlayer;
Adafruit_LIS3MDL lis3mdl; // (i2c)
rampUnsignedInt wingRamp;
Servo myServo;

constexpr int LED_PINS[] = {0, 1, 2, 3, 4, 5, 6}; 
...
int pulseWidth = 750; // Pulse width in microseconds
...
void setup() {
  ...
  DEBUG_PRINTLN("Initial setup complete.", 2000);
}
...
void parkServo() {
  DEBUG_PRINTLN("Moving servo to parked position.", 10);
...
void checkButton() {
  ...
  int currentButtonState = digitalRead(SOUND_POT_PIN);
